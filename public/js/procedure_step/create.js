...
function getSchoolEducators() {
    ...
    $.ajax({
        type: 'GET',
        url: page.siteRoot() + 'procedure_steps/getSchoolEducators/' + procedure_id,
        success: function (result) {
            if (result.statusCode === 200) {
                if (result['educators'].length === 0) {
                    ...
                    $.each(eval(result['educators']), function () {
                        $approver_user_ids.append("<option value='" + this['id'] + "'>" + this['name'] + "</option>");
                        $related_user_ids.append("<option value='" + this['id'] + "'>" + this['name'] + "</option>");
                    });
                }
            }
            return false;
