...
class WXBizMsgCrypt {
     ...
     * </ol>
     *
     * @param $sReplyMsg string 公众平台待回复用户的消息，xml格式的字符串
     * @param $sTimeStamp string 时间戳，可以自己生成，也可以用URL参数的timestamp
     * @param $sNonce string 随机串，可以自己生成，也可以用URL参数的nonce
     * @param &$sEncryptMsg string 加密后的可以直接回复用户的密文，包括msg_signature, timestamp, nonce, encrypt的xml格式的字符串,
     *                      当return返回0时有效
     *
     ...
     * </ol>
     *
     * @param $sMsgSignature string 签名串，对应URL参数的msg_signature
     * @param $sTimeStamp string 时间戳 对应URL参数的timestamp
     * @param $sNonce string 随机串，对应URL参数的nonce
     * @param $sPostData string 密文，对应POST请求的数据
     * @param &$sMsg string 解密后的原文，当return返回0时有效
     *
     * @return int 成功0，失败返回对应的错误码
     */
    public function DecryptMsg($sMsgSignature, $sNonce, $sPostData, &$sMsg, $sTimeStamp = null) {
